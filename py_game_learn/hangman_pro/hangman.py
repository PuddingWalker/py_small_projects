...
WORDS = {'Colors':'red orange yellow green blue indigo violet white black brown'.split(),
         ...
         'Animals':'bat bear beaver cat cougar crab deer dog donkey duck eagle fish frog goat leech lion lizard monkey moose mouse otter owl panda python rabbit rat sark sheep skunk squid tiger turkey turtle weasel whale wolf wombat zebra'.split()}
...
class Hangman():
    def __init__(self):
        ...
        self.missedLetters = []
        self.correctLetters = []
        self.secretWord, self.secretSet = self.getRandomWord(WORDS)
        self.gameIsDone = False

    def getRandomWord(self, wordList):
        wordKey = random.choice(list(WORDS.keys()))
        return random.choice(WORDS[wordKey]),wordKey

    def displayBoard(self):
        print(f'The secret word is in the set {self.secretSet}')
